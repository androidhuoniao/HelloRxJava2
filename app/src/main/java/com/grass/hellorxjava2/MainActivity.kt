    ...
    fun test() {
        ...
        Observable.create(object : ObservableOnSubscribe<Int> {
            override fun subscribe(emitter: ObservableEmitter<Int>) {
                ...
                Log.i(TAG, "test is working1")
                emitter.onNext(1)
                Log.i(TAG, "test is working2")
                emitter.onNext(2)
//                emitter.onComplete()
            }
        })
    ...
    fun <T> uploadLogError(tag: String?, msg: String): ObservableTransformer<T, T>? {
        return ObservableTransformer { source -> source.doOnError { throwable -> Log.i(TAG, msg + ": " + throwable.message.toString()) } }
    }
}